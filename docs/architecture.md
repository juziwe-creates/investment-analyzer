...
Provider symbols are still derived from the transaction-derived ticker and exchange fields. For EODHD the app normalizes common symbols into the provider's `{symbol}.{exchange}` format, such as `AAPL.US`, `MSFT.US`, or `MUV2.XETRA`. German transaction tickers ending in `.DE` or `.DEX` are treated as Xetra symbols for this adapter. A future enrichment step should add provider-symbol validation or a separate provider symbol mapping so analytics do not depend on manually perfect tickers.
