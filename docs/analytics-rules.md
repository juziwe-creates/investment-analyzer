...
Investment History derives cumulative recorded dividends from the same lot inventory engine and gross-first cash convention. Value mode keeps dividends separate from Position Value. Economic Value mode adds that cumulative cash to remaining market value and cumulative attributed sale proceeds. No provider dividend events are substituted for personal transaction cash.
...
- Value charts show remaining benchmark units times the dated level, in EUR. Economic Value charts add cumulative counterfactual exit proceeds to that remaining value. Total-return benchmark dividends remain embedded in the level and are never added again.
- Investment History and Portfolio Performance expose the same Value and Economic Value modes. The actual Investment Economic Value timeline is remaining position market value plus cumulative attributed sale proceeds plus cumulative recorded dividends. Value mode never includes those realized cash flows.
- Users may select multiple benchmark overlays. The first selected benchmark remains the table and annual-grid context; every selected benchmark is rendered in the historical chart. Benchmark selections are URL-persisted.
- Mode and benchmark changes reuse the existing `TimeViewportProvider`. Primary actual-series dates alone define navigation, so the selected range, zoom, pan, presets and navigator universe remain stable.
- Portfolio Economic Value is calculated only when no sale or dividend cash precedes a later purchase. In that case, current data cannot establish whether realized cash was withdrawn or redeployed. The chart discloses and withholds Economic Value instead of summing lifetime proceeds into newly purchased assets and double-counting capital.
