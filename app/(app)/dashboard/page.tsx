...
import { DashboardPerformance, DashboardCapitalDeployment, DashboardHoldings, HistoryLoading } from "@/components/dashboard-history";
...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const [{ data: allTransactions, error: transactionsError }, { data: allLatest, error: latestError }, { data: allManual, error: manualError }] = await Promise.all([transactionsQuery, measureAnalytics("dashboard.latest-prices.read", () => latestQuery), measureAnalytics("dashboard.manual-prices.read", () => manualQuery)]);
  ...
  const { holdings, summary, lots } = await measureAnalytics("dashboard.current.calculate", () => buildCurrentAnalytics(filteredTransactions, latest, manual));
  const { canAggregate: currencyReady, unsupportedCurrencies } = eurAggregationStatus([
    ...
    ...latest.map((price) => marketDataCurrency({ fallbackCurrency: price.currency, providerId: price.provider, providerSymbol: price.provider_symbol }))
  ]);
