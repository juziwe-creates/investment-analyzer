...
import { DashboardPerformance, DashboardCapitalDeployment, HistoryLoading } from "@/components/dashboard-history";
...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const { holdings, summary } = await measureAnalytics("dashboard.current.calculate", () => buildCurrentAnalytics(filteredTransactions, latest, manual));
