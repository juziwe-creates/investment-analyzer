...
export default async function DividendsPage({ searchParams }: { searchParams: Promise<{ portfolio?: string }> }) {
  ...
  const { data, error } = await presentationTransactions(portfolioId);
  const dividends = (data ?? []).filter((row) => row.type === "dividend").toReversed();
  const yields = investmentYieldsOnCost(data ?? [], new Date().toISOString().slice(0, 10));
  const yieldYear = new Date().getUTCFullYear() - 1;
