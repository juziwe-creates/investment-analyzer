...
export const quote = (date: string, value: number, id: BenchmarkId = "msci-world"): BenchmarkObservation => ({
  ...
  currency: "EUR", series_type: "net_total_return", is_derived: true, is_partial_period: false
});
...
export function sourceLots(transactions: AnalyticsTransaction[], matching: LotMatchingMethod = "fifo", end = "2022-01-03"): LotProfitability[] {
  return calculatePurchaseLots(transactions, ["A", "B"].map((key) => ({ security_key: key, price_date: end, price: 200, currency: "EUR" })), end, { lotMatchingMethod: matching }).map((lot) => ({
    ...
    quantity: lot.originalQuantity, remainingQuantity: lot.remainingQuantity, buyPrice: lot.acquisitionCostPerShare,
