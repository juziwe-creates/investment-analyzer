...
export function PurchaseLotsTable({ lots }: { lots: LotProfitability[] }) {
  ...
  return <section className="space-y-4" aria-labelledby="purchase-lots-heading">
    ...
    {rows.length === 0 ? <div className="alpha-surface flex h-36 items-center justify-center text-sm text-muted-foreground">No {tab === "all" ? "" : `${tab} `}purchase lots in this period.</div> : <>
    ...
    <p className="text-xs leading-5 text-muted-foreground">The visible period filters purchase dates only; returns retain their current valuation. Canonical Total Return, dividend-tax treatment, and cross-view lot-matching policy remain pending product decisions. Yield on Cost is defined at investment level, not per purchase lot.</p>
  </section>;
}
