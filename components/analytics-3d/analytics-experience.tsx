...
export function AnalyticsExperience({ model }: { model: UniverseModel }) {
  ...
  return <section ref={wrapper} className={`${styles.experience} ${immersive ? styles.immersive : ""}`} aria-label="Portfolio universe" onKeyDown={(event) => {
    ...
    <div className={styles.workspace}>
      <div className={`${styles.stage} ${small && mode === "universe" && !info ? styles.compact : ""}`}>
        {!ready ? <div className={styles.loading}><AlphaProgress status="Preparing portfolio universe" /></div> : small ? <div className={styles.mobileCore} aria-hidden="true">α</div> : contextLost ? fallback : <SceneBoundary fallback={fallback}><Scene model={model} mode={mode} selected={selected} hovered={hovered} reducedMotion={reduced} reset={reset} onSelect={setSelected} onHover={setHovered} onContextLost={() => setContextLost(true)} fallback={fallback} /></SceneBoundary>}
    ...
    <footer className={styles.footer}><span>Latest stored quotes · {formatDate(model.asOfDate)}</span><span>Size: value · Color: unrealized gain</span></footer>
    ...
    <div className={styles.explorer}>
      ...
      <label htmlFor="universe-holding">Explore investments</label><select ref={selector} id="universe-holding" value={selected ?? ""} onChange={(event) => { setSelected(event.target.value || null); setMode("universe"); }}><option value="">Choose investment</option>{model.holdings.map((item) => <option key={item.key} value={item.key}>{item.name}</option>)}</select>
