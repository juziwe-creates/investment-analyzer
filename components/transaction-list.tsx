...
export function TransactionList({ transactions, analyticsRows, page, pageCount, previousHref, nextHref, canDelete = false, portfolioId, yields = {} }: { canDelete?: boolean; portfolioId?: string; yields?: Record<string, YieldOnCost>; transactions: Transaction[]; analyticsRows: TransactionAnalyticsRow[]; page: number; pageCount: number; previousHref: string | null; nextHref: string | null }) {
  ...
  const deleted = useCallback(() => { setSelected(null); setFeedback("Transaction deleted"); router.refresh(); }, [router]);
  ...
  const investmentYield = selected ? yields[selected.isin ?? selected.ticker ?? selected.security_name] : undefined;
  ...
  return <section className="space-y-4" aria-labelledby="transaction-history-heading"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 id="transaction-history-heading" className="alpha-section-title">Transaction history</h2><p className="mt-1 text-sm text-muted-foreground">Page {page} of {pageCount}. Select a row for source and decision context.</p></div><label className="relative block w-full sm:w-72"><span className="sr-only">Filter current page</span><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Filter current page" className="pl-9" /></label></div>
    ...
    <DecisionDrawer open={selected !== null} onClose={close} eyebrow={selected?.type.toUpperCase() ?? "Transaction"} title={selected?.security_name ?? "Transaction"} subtitle={selected ? formatDate(selected.trade_date) : undefined} metrics={drawerMetrics} note={selectedAnalytics ? "Current-model lot values use the existing LIFO analytical view. Canonical Total Return, dividend tax treatment, and cross-view lot policy remain pending. Yield on Cost is investment-level." : "This source-of-truth transaction has no separate lot analysis."}>{canDelete && selected ? <DeleteTransactionControl key={selected.id} transactionId={selected.id} portfolioId={portfolioId} onDeleted={deleted} /> : null}</DecisionDrawer>
