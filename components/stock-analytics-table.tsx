...
export function StockAnalyticsTable({ rows }: StockAnalyticsTableProps) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows
      .filter((row) => {
        ...
        const matchesFrom = !fromDate || row.firstPurchaseDate >= fromDate;
        const matchesTo = !toDate || row.firstPurchaseDate <= toDate;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Purchase-lot profitability aggregated by security.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {rows.length === 0 ? (
          <div className="flex h-48 items-center justify-center rounded-md border border-dashed text-center text-sm text-muted-foreground">
            Add buy transactions to analyze stock-level profitability.
          </div>
        ) : (
          <div className="space-y-4">
            <div className="grid gap-3 rounded-md border bg-muted/30 p-3 md:grid-cols-[minmax(220px,1fr)_180px_180px_auto] md:items-end">
              ...
              <label className="space-y-1 text-sm">
                <span className="text-muted-foreground">First purchase from</span>
                <Input
                  type="date"
                  value={fromDate}
              ...
              <label className="space-y-1 text-sm">
                <span className="text-muted-foreground">First purchase to</span>
                <Input
                  type="date"
                  value={toDate}
              ...
              <table className="w-full min-w-[1540px] text-sm">
                  <thead>
                    <tr className="border-b text-left text-muted-foreground">
                      ...
                      {sortableHeader("First buy", "firstPurchaseDate", "left")}
                      {sortableHeader("Buys", "transactionCount")}
                  ...
                  <tbody>
                    {filteredRows.map((row) => (
                      <tr key={row.securityKey} className="border-b last:border-0">
                        <td className="px-3 py-3 font-medium">{row.securityName}</td>
                        <td className="px-3 py-3">{formatDate(row.firstPurchaseDate)}</td>
